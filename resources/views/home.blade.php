...
function updateCartPreview() {
    ...
    let discountedTotal = totalSum;
    if (discountType === 'percentage') discountedTotal -= (totalSum * discountValue / 100);
    else if (discountType === 'flat') discountedTotal -= discountValue;
    if (discountedTotal < 0) discountedTotal = 0;
...
finalForm.addEventListener('submit', function (e) {
    ...
    const payload = {
        ...
        payment_method: productsList[0].paymentMethod
    };
    ...
    .then(data => {
        if (data.success) {
            ...
            showPrompt('❌ ' + (data.message || 'Failed to complete sale'), 'error');
        }
    })
    .catch(err => {
        console.error(err);
